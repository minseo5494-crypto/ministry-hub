...
const checkUploadRateLimit = async (
  ...
  // 제한: 시간당 20곡, 일일 100곡
  if ((hourlyCount || 0) >= 20) {
    return { allowed: false, message: 'uploadHourlyLimit' }
  }
  if ((dailyCount || 0) >= 100) {
    return { allowed: false, message: 'uploadDailyLimit' }
  }
...
const notebookPageToEditorSong = (page: NotebookPage): EditorSong => {
  ...
  let songName = page.pageType === 'sheet' ? (page.songName || t('noTitle')) :
                 page.pageType === 'blank' ? t('blankPage') :
                 page.pageType === 'staff' ? t('staffPage') :
                 (page.uploadFileName || t('uploadPage'))
...
// ✅ 곡 삭제
  ...
  if (!confirm(t('deleteConfirm', { name: song.song_name }))) {
    return
  }
...
setNewSong({ ...newSong, tempo: tempoValue })
  ...
  if (duplicateSongs.length > 0) {
    ...
    const confirmed = confirm(
      t('duplicateConfirm', { list: duplicateInfo })
    )
  ...
  if (!rateLimit.allowed) {
    alert(t(rateLimit.message as any))
    return
  }
  ...
  try {
    ...
    alert(t('songAddFailed', { message: error.message }))
  } finally {
    setUploading(false)
  }
}
    ...
    <div className="my-page-container flex min-h-screen bg-[#F8FAFC]">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex-col h-screen sticky top-0 hidden lg:flex">
        ...
        <div className="px-4 pb-3">
          <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider">{t('title')}</h2>
        </div>
        ...
        {/* 통계 카드 */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 flex flex-col gap-1 border border-slate-100 shadow-sm rounded-2xl">
              <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-2">{t('statUploaded')}</span>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-slate-900">{totalSongCount.toLocaleString()}</span>
                <span className="text-xs text-slate-400 font-medium">{t('statTotal')}</span>
              </div>
            </div>

            <div className="bg-white p-6 flex flex-col gap-1 border border-slate-100 shadow-sm rounded-2xl">
              <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-2">{t('statPublic')}</span>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-slate-900">{publicSongCount.toLocaleString()}</span>
                <span className="text-xs text-emerald-500 font-medium">{t('statPublicActive')}</span>
              </div>
            </div>

            <div className="bg-white p-6 flex flex-col gap-1 border border-slate-100 shadow-sm rounded-2xl">
              <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-2">{t('statTeamShared')}</span>
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-slate-900">{teamSongCount.toLocaleString()}</span>
                <span className="text-xs text-slate-400 font-medium">{t('statTeamCount', { count: userTeams.length })}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="flex flex-col lg:flex-row items-center gap-4">
                ...
                <div className="flex items-center gap-3 w-full lg:w-auto">
                  ...
                  >
                    ...
                    <option value="teams">{t('filterTeam')}</option>
            ...
            <div className="hidden lg:flex items-center justify-between py-2 px-6">
              <h2 className="text-lg font-bold text-slate-900">
                {activeTab === 'uploaded' && t('desktopTabTitle_uploaded', { count: filteredSongs.length })}
                {activeTab === 'liked' && t('desktopTabTitle_liked', { count: likedSongs.length })}
                {activeTab === 'notes' && t('desktopTabTitle_notes', { count: notebooks.length })}
                {activeTab === 'bookmarks' && t('desktopTabTitle_bookmarks', { count: bookmarkedSetlists.length })}
                {activeTab === 'downloads' && t('desktopTabTitle_downloads', { count: downloadHistories.length })}
              </h2>
            </div>
                            ...
                            {song.usage_count !== undefined && song.usage_count > 0 && (
                              <div className="mt-1.5 flex items-center gap-2 text-[10px] text-slate-400">
                                <span className="flex items-center gap-1">
                                  ...
                                  {t('usageCount', { count: song.usage_count })}
                                </span>
                                {song.usage_count_last_30_days !== undefined && song.usage_count_last_30_days > 0 && (
                                  <span>{t('usageRecent30', { count: song.usage_count_last_30_days })}</span>
                                )}
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex gap-1 md:gap-2 ml-2 md:ml-4 flex-shrink-0">
                            ...
                            {song.file_url && (
                              <button
                                ...
                                title={previewStates[song.id] ? t('collapse') : t('preview')}
                              >
                        ...
                        {previewStates[song.id] && song.file_url && (
                          <div className="mt-4 pt-4 border-t border-slate-100">
                            <div className="flex items-center justify-between mb-2">
                              <h4 className="font-semibold text-slate-700 text-sm">{t('sheetMusic')}</h4>
                              <span className="text-xs text-slate-400">{t('doubleClickZoom')}</span>
                            </div>
                            {song.file_type === 'pdf' ? (
                              <iframe
                ...
                {filteredSongs.length > itemsPerPage && (
                  <div className="flex items-center justify-center gap-2 p-6 border-t border-slate-100">
                    ...
                    <span className="ml-4 text-sm text-slate-500">
                      {t('paginationInfo', { total: filteredSongs.length.toLocaleString(), from: ((currentPage - 1) * itemsPerPage) + 1, to: Math.min(currentPage * itemsPerPage, filteredSongs.length) })}
                    </span>
                  </div>
                )}
              </>
              ...
              <>
                {loadingLiked ? (
                  <div className="text-center py-12">
                    ...
                    <p className="mt-4 text-slate-500">{t('loadingData')}</p>
                  </div>
                ) : likedSongs.length === 0 ? (
                  <div className="text-center py-12 text-slate-500">
                    <Heart className="w-12 h-12 mx-auto mb-4 text-slate-300" />
                    <p className="text-lg font-medium">{t('noLikedSongs')}</p>
                    <p className="text-sm mt-2">{t('noLikedSongsHint')}</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-100">
                    {likedSongs.map((song) => (
              ...
              <>
                {notebooksLoading ? (
                  ...
                  <div className="text-center py-12 text-slate-500">
                    ...
                    <p className="text-lg font-medium">{t('noNotebooks')}</p>
                    <p className="text-sm mt-2">{t('noNotebooksHint')}</p>
                  </div>
                ) : (
                  <>
                    {/* 📒 상단 툴바 */}
                    <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
                      ...
                      <span className="text-sm text-slate-400">{t('notebookCount', { count: notebooks.length })}</span>
                    </div>
                          ...
                          >
                            ...
                            <div className="px-2 pt-2 pb-1">
                              ...
                              <p className="text-[10px] text-slate-400">
                                {t('notebookPages', { count: nb.pages.length })} · {new Date(nb.updated_at).toLocaleDateString()}
                              </p>
                              {/* 버튼 */}
                              <div className="flex mt-1 border-t border-slate-100 pt-1">
                                ...
                                <button
                                  onClick={async () => {
                                    if (!confirm(t('notebookDeleteConfirm', { title: nb.title }))) return
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-slate-500 truncate">
                            {t('authorName', { name: setlist.author_name })}{setlist.author_church ? ` · ${setlist.author_church}` : ''}
                          </p>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs text-slate-400">{t('songCountUnit', { count: setlist.songs.length })}</span>
              ...
              <>
                {loadingDownloads ? (
                  ...
                  <div className="text-center py-12 px-6">
                    ...
                    <p className="text-slate-500 font-medium">{t('noDownloads')}</p>
                    <p className="text-sm text-slate-400 mt-1">{t('noDownloadsHint')}</p>
                  </div>
                ) : (
                  <div className="divide-y divide-slate-100">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-slate-500 mt-0.5 truncate">
                              {displayNames.join(', ')}
                              {extraCount > 0 && ` ${t('extraSongs', { count: extraCount })}`}
                            </p>
                          </div>
                          <button
                            onClick={async (e) => {
                            ...
                            style={{ touchAction: 'manipulation' }}
                            aria-label={t('deleteDownloadLabel')}
                          >
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
          ...
          <footer className="mt-12 text-center text-slate-400 text-xs">
            <div className="flex items-center justify-center gap-6 mb-4">
              <a href="#" className="hover:text-indigo-600 transition-colors">{t('footerTerms')}</a>
              <a href="#" className="hover:text-indigo-600 transition-colors">{t('footerPrivacy')}</a>
              <a href="#" className="hover:text-indigo-600 transition-colors">{t('footerCopyright')}</a>
              <a href="#" className="hover:text-indigo-600 transition-colors">{t('footerContact')}</a>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg w-full max-w-2xl p-6 my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-900">{t('addSongTitle')}</h2>
              <button
                onClick={() => {
                  setShowAddSongModal(false)
                  setNewSong({
            ...
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
              <div>
                ...
                {/* 🔍 중복 경고 표시 */}
                {checkingDuplicate && (
                  <p className="mt-1 text-sm text-slate-500">{t('duplicateChecking')}</p>
                )}
                {!checkingDuplicate && duplicateSongs.length > 0 && (
                  <div className="mt-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <p className="text-sm font-medium text-orange-800 mb-1">
                      ⚠️ {t('duplicateWarning', { count: duplicateSongs.length })}
                    </p>
                    <ul className="text-sm text-orange-700 space-y-1">
                      ...
                      {duplicateSongs.length > 5 && (
                        <li className="text-orange-600">{t('duplicateMore', { count: duplicateSongs.length - 5 })}</li>
                      )}
                    </ul>
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-2">
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        alert(t('visibilityPublicWarning'))
                      }}
                      className="mr-3 accent-blue-500"
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${newSong.visibility === 'public' ? 'text-indigo-700' : 'text-slate-900'}`}>{t('visibilityPublic')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityPublicDesc')}</div>
                    </div>
                    {newSong.visibility === 'public' && (
                  ...
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${newSong.visibility === 'teams' ? 'text-violet-700' : 'text-slate-900'}`}>{t('visibilityTeam')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityTeamDesc')}</div>
                    </div>
                    {newSong.visibility === 'teams' && (
                  ...
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${newSong.visibility === 'private' ? 'text-slate-700' : 'text-slate-900'}`}>{t('visibilityPrivate')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityPrivateDesc')}</div>
                    </div>
                    {newSong.visibility === 'private' && (
                ...
                {newSong.visibility === 'teams' && (
                  <div className="mt-3">
                    <label className="block text-sm font-medium text-slate-700 mb-2">
                      {t('selectTeamLabel')} <span className="text-red-500">*</span>
                    </label>
                    {userTeams.length > 0 ? (
                      ...
                      <p className="text-sm text-slate-500">{t('noTeams')}</p>
                    )}
                  </div>
                )}
              </div>
...
className="w-full px-3 py-2 border border-slate-200 rounded-lg"
>
<option value="">{t('keySelect')}</option>
{KEYS.map(key => (
<option key={key} value={key}>{key}{newSong.key.includes('m') ? 'm' : ''}</option>
                ...
                {/* 박자 */}
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t('timeSignatureLabel')}</label>
                  <select
                ...
                {/* 템포 */}
<div>
<label className="block text-sm font-medium text-slate-700 mb-1">{t('tempoLabel')}</label>
<select
value={newSong.tempo}
...
/>
              ...
              {/* 시즌 선택 */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">{t('seasonLabel')}</label>
                <select
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {themesLoading ? (
                    <p className="text-sm text-slate-500">{t('themeLoading')}</p>
                  ) : (
                ...
                <div className="flex gap-2 mt-3">
                  <input
                    ...
                    placeholder={t('newThemePlaceholder')}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                  {t('sheetFileLabel')} <span className="text-red-500">*</span>
                </label>
                <div className="mt-1">
                  <input
                    ref={fileInputRef}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">
              {t('renameDesc', { name: renameNote.song_name })}
            </p>
            <input
              type="text"
              value={newTitle}
      ...
      {showNotebookRenameModal && renamingNotebook && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            <h2 className="text-xl font-bold mb-4">{t('notebookRenameTitle')}</h2>
            <input
              type="text"
              value={newNotebookTitle}
              onChange={(e) => setNewNotebookTitle(e.target.value)}
              placeholder={t('notebookRenamePlaceholder')}
              className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
              style={{ fontSize: '16px' }}
              autoFocus
            />
              ...
              <button
                onClick={async () => {
                  if (!newNotebookTitle.trim()) {
                    alert(t('notebookRenameRequired'))
                    return
                  }
      ...
      {showShareModal2 && shareNote && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h2 className="text-xl font-bold mb-4">{t('exportTitle')}</h2>
            <p className="text-sm text-slate-600 mb-4">
              {t('exportDesc')}
            </p>
            ...
            {/* 파일명 입력 */}
            <div className="mb-4">
              <label className="block text-sm font-medium text-slate-700 mb-2">{t('fileNameLabel')}</label>
              <input
                type="text"
                value={shareFileName}
            ...
            <div className="mb-4">
                  ...
                  try {
                    ...
                    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
                      await navigator.share({
                        title: shareFileName,
                        text: t('shareText', { name: shareNote.song_name }),
                        files: [file]
                      })
                    } else {
              ...
              >
                ...
                <span className="text-xs text-red-500 mt-1">{t('sharePdf')}</span>
              </button>
              ...
              >
                ...
                <span className="font-medium text-indigo-700">{t('imageLabel')}</span>
                <span className="text-xs text-indigo-500 mt-1">{t('sharePng')}</span>
              </button>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <span className="font-medium text-slate-700 text-sm">{t('imageLabel')}</span>
                </button>
              </div>
            </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg w-full max-w-2xl p-6 my-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-slate-900">{t('editSongTitle')}</h2>
              <button
                onClick={() => {
                  setShowEditModal(false)
                  setEditingSongId(null)
              ...
              <div className="border-t pt-4">
                ...
                <div className="space-y-2">
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${editSong.visibility === 'public' ? 'text-indigo-700' : 'text-slate-900'}`}>{t('visibilityPublic')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityPublicDesc')}</div>
                    </div>
                    {editSong.visibility === 'public' && (
                      <span className="text-indigo-500 text-xl">✓</span>
                  ...
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${editSong.visibility === 'teams' ? 'text-violet-700' : 'text-slate-900'}`}>{t('visibilityTeam')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityTeamDesc')}</div>
                    </div>
                    {editSong.visibility === 'teams' && (
                      <span className="text-violet-500 text-xl">✓</span>
                  ...
                  <label className={`flex items-center p-3 border-2 rounded-lg cursor-pointer transition-all ${
                    ...
                    />
                    <div className="flex-1">
                      <div className={`font-medium ${editSong.visibility === 'private' ? 'text-slate-700' : 'text-slate-900'}`}>{t('visibilityPrivate')}</div>
                      <div className="text-sm text-slate-500">{t('visibilityPrivateDesc')}</div>
                    </div>
                    {editSong.visibility === 'private' && (
                      <span className="text-slate-500 text-xl">✓</span>
                ...
                {editSong.visibility === 'teams' && (
                  <div className="mt-3">
                    ...
                    {userTeams.length > 0 ? (
                      ...
                      <p className="text-sm text-slate-500">{t('noTeamsShort')}</p>
                    )}
                  </div>
                )}
              </div>
                  ...
                  {editCurrentFileUrl && !editFile && (
                    <div className="mb-2 p-3 bg-slate-50 rounded-lg flex items-center justify-between">
                      <div className="flex items-center gap-2 text-sm text-slate-600">
                        <FileText size={16} />
                        <span>{t('currentFile', { name: editCurrentFileUrl.split('/').pop()?.substring(0, 30) || '' })}</span>
                      </div>
                      <button
