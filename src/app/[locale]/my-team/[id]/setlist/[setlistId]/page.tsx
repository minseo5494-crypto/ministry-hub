...
import ShareSetlistModal from '@/app/[locale]/community/components/ShareSetlistModal'
      ...
      <div className="px-2 py-3 md:p-6 space-y-2.5 md:space-y-4">
        ...
        <div className="flex items-center gap-1.5 md:gap-3">
          {(song.songs.lyrics || song.songs.file_url) && (
            <button
              ...
              title={isPreviewOpen ? t('collapse') : t('expand')}
            >
          ...
          <button
            ...
            title={song.songs.youtube_url ? (isYoutubeOpen ? t('youtubeCollapse') : t('youtube')) : t('youtubeNoLink')}
          >
        ...
        {song.notes ? (
          <div className="bg-amber-50/30 border border-amber-100/50 rounded-lg p-3 md:p-4">
            ...
            {song.notes.length > 150 && (
              ...
              >
                ...
                {isNoteExpanded ? t('lessMemo') : t('moreMemo')}
              </button>
            )}
          </div>
        ) : (
          canEdit && (
            ...
            >
              <span className="material-symbols-outlined text-sm">add</span> {t('addMemo')}
            </button>
          )
        )}
      </div>
      ...
      {isPreviewOpen && (
        <div className="border-t border-gray-100 bg-gray-50/50 p-6">
          {song.songs.lyrics && (
            <div className="mb-4">
              <h4 className="text-sm font-bold text-gray-700 mb-2 flex items-center gap-1">
                <span className="material-symbols-outlined text-base">lyrics</span> {t('lyrics')}
              </h4>
          ...
          {song.songs.file_url && (
            <div>
              <h4 className="text-sm font-bold text-gray-700 mb-2 flex items-center gap-1">
                <span className="material-symbols-outlined text-base">description</span> {t('sheetMusic')}
                <span className="text-xs text-gray-400 font-normal ml-1">{t('doubleTapFullscreen')}</span>
              </h4>
              <div
      ...
      {isYoutubeOpen && song.songs.youtube_url && (
        <div className="border-t border-gray-100 bg-gray-50/50 p-6">
          ...
          {getYoutubeEmbedUrl(song.songs.youtube_url) ? (
            ...
            <p className="text-sm text-gray-500">{t('invalidYoutubeLink')}</p>
          )}
        </div>
      )}
    </div>
...
const canEdit = () => {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      alert(t('editFailed', { message: error.message }))
    }
  }
  ...
  const handleSaveGuide = async () => {
    ...
    try {
      ...
      alert(t('guideSaveFailed', { message: error.message }))
    } finally {
  ...
  const handleDeleteSetlist = async () => {
    if (!canEdit()) {
      alert(t('deletePermDenied'))
      return
    }

    if (!confirm(t('deleteConfirmSetlist'))) return
  ...
  const removeSongFromSetlist = async (songId: string) => {
    ...
    if (!confirm(t('removeSongConfirm'))) return
    ...
    try {
      ...
      alert(t('removeFailed', { message: error.message }))
    }
  }

  const openAddSongModal = async () => {
  if (!canEdit()) {
    alert(t('addPermDenied'))
    return
  }
  ...
  const addSongToSetlist = async (song: Song) => {
    try {
      ...
      alert(t('addSongFailed', { message: error.message }))
    }
  }
...
const removeSongForm = (index: number) => {
  ...
  const saveSongForm = async () => {
    ...
    try {
      ...
      alert(t('songFormSaved'))
      setShowSongFormModal(false)
      fetchSetlistDetail()
    } catch (error: any) {
      console.error('Error saving song form:', error)
      alert(t('songFormSaveFailed', { message: error.message }))
    }
  }
  ...
  try {
    ...
    const koreanFont = 'NanumGothic'
      ...
      alert(t('pptDownloaded'))
    } catch (error: any) {
      console.error('Error generating PPT:', error)
      alert(t('pptGenerateFailed', { message: error.message }))
    } finally {
  ...
  const handleSaveSheetNotes = async (dataList: Array<{
    ...
    if (result) {
      ...
      try {
        ...
        if (!existingNotebook) {
          ...
          const nb = await createNotebookFromSetlist({
            ...
            setlistTitle: setlist?.title || 'Untitled Setlist',
      ...
      alert(`${t('setlistNoteSaved', { count: savedSongCount })}${notebookMsg}`)
    } else {
      ...
      <aside className="hidden lg:flex w-64 flex-shrink-0 bg-white border-r border-gray-200 flex-col sticky top-0 h-screen overflow-hidden">
        ...
        <nav className="flex-1 overflow-y-auto p-2">
          ...
          {otherSetlists.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-8">{t('noSetlistsSidebar')}</p>
          )}
        </nav>
        {/* 뒤로가기 버튼 + 사용자 프로필 */}
                ...
                {!isEditing && (
                  <div className="flex items-center gap-3 text-xs text-gray-500 ml-0 lg:ml-0 mt-2 md:mt-0">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {setlist.service_type || t('worship')}
                    </span>
                    <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                    <span className="flex items-center gap-1">
                      <span className="material-symbols-outlined text-sm">music_note</span>
                      {t('songCountUnit', { count: songs.length })}
                    </span>
                  </div>
                )}
              </div>
                ...
                {isEditing ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">{downloadingPPT ? t('pptGenerating') : 'PPT'}</span>
                    </button>
                                        <button
