  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMsg.includes('already registered') || errorMsg.includes('Already registered')) {
        ...
        setError(t('auth.passwordCondition'))
      } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('auth.churchPlaceholder')}
                className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-base"
              />
            </div>
          </div>
          ...
          <div className="text-xs text-gray-600 space-y-3">
            ...
            >
              ...
              <span>
                <Link href="/terms" className="text-blue-600 hover:text-blue-700" onClick={(e) => e.stopPropagation()}>{t('footer.terms')}</Link>
                {' & '}
                <Link href="/privacy" className="text-blue-600 hover:text-blue-700" onClick={(e) => e.stopPropagation()}>{t('footer.privacy')}</Link>
                {' '}{t('auth.agreeTermsText', { terms: '', privacy: '' }).replace(/\s+/g, ' ').trim() ? '' : ''}
            ...
            >
              ...
              <span>
                <Link href="/copyright" className="text-blue-600 hover:text-blue-700" onClick={(e) => e.stopPropagation()}>{t('footer.copyright')}</Link>
                {' '}{t('auth.agreeCopyrightText', { copyright: '' }).replace(/\s+/g, ' ').trim() ? '' : ''}
                <span className="text-red-500"> *</span>
              </span>
            </div>
          </div>
