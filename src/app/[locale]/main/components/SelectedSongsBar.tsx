...
import { FolderOpen, FileText, Presentation, BookOpen } from 'lucide-react'
...
type SelectedSongsBarProps = {
  ...
  onSaveToNotebook: () => Promise<boolean>
}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 w-full sm:w-auto">
            <span className="text-xs sm:text-sm text-gray-600 whitespace-nowrap">
              {t('selectedCount', { count: selectedSongs.length })}
            </span>
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              className={`flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-amber-500 text-white rounded-lg hover:bg-amber-600 text-xs sm:text-sm flex items-center justify-center whitespace-nowrap ${savingNotebook ? 'opacity-75 cursor-not-allowed' : ''}`}
            >
              {savingNotebook ? (
                <>
            ...
            >
              {downloadingPDF ? (
                <>
                  <div className="animate-spin rounded-full h-3 w-3 sm:h-4 sm:w-4 border-b-2 border-white mr-1 sm:mr-2"></div>
                  <span className="hidden sm:inline">{t('creatingPDF')}</span>
                  <span className="sm:hidden">{t('creatingShort')}</span>
                </>
              ) : (
                <>
            ...
            >
              {downloadingPPT ? (
                <>
                  <div className="animate-spin rounded-full h-3 w-3 sm:h-4 sm:w-4 border-b-2 border-white mr-1 sm:mr-2"></div>
                  <span className="hidden sm:inline">{t('creatingPPT')}</span>
                  <span className="sm:hidden">{t('creatingShort')}</span>
                </>
              ) : (
                <>
