...
export default function AdminApprovalsPage() {
  ...
  const handleApprove = async (request: ApprovalRequest) => {
    if (!confirm(t('teamApproveConfirm', { name: request.team_name }))) {
      return
    }
    ...
    try {
      ...
      alert(t('teamApproved', { name: request.team_name, code: newTeam.invite_code }))
      fetchRequests()
    } catch (error: any) {
      console.error('Error approving request:', error)
      alert(t('teamApproveFailed', { message: error.message }))
    } finally {
      setProcessing(null)
    }
  }
  ...
  const handleReject = async (request: ApprovalRequest) => {
    const reason = prompt(t('teamRejectReasonPrompt'))
    ...
    try {
      ...
      alert(t('teamRejected'))
      fetchRequests()
    } catch (error: any) {
      console.error('Error rejecting request:', error)
      alert(t('teamRejectFailed', { message: error.message }))
    } finally {
      setProcessing(null)
    }
  }
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">{t('teamApprovalTitle')}</h1>
            <button
              onClick={() => router.push('/')}
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-md p-4 mb-6">
          <div className="flex gap-2">
            ...
            >
              {t('filterAllCount', { count: requests.length })}
            </button>
            <button
        ...
        {requests.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-600">{t('noTeamApprovalRequests')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      >
                        {request.status === 'pending'
                          ? t('statusPendingEmoji')
                          : request.status === 'approved'
                          ? t('statusApprovedEmoji')
                          : t('statusRejectedEmoji')}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div className="flex items-center text-gray-700">
                        ...
                        <span className="text-sm">
                          {request.team_type === 'church_internal' ? 'Church Internal' : 'External'}
                        </span>
                      </div>
                      {request.church_name && (
                        <div className="flex items-center text-gray-700">
                    ...
                    <div className="text-sm text-gray-600">
                      <p>
                        {t('requesterLabel', { info: request.users?.email || t('requesterUnknown') })}
                      </p>
                      <p>
                        {t('requestDateLabel', { date: new Date(request.created_at).toLocaleString() })}
                      </p>
                    </div>

                    {!request.requester_id && request.status === 'pending' && (
                      <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded text-sm text-amber-700">
                        {t('requesterDeletedWarning')}
                      </div>
                    )}
                  </div>
