...
export default function SongApprovalsPage() {
  ...
  const handleApprove = async (request: SongApprovalRequest) => {
    if (!request.requester_id) {
      alert(t('requesterDeletedCannotApprove'))
      return
    }

    if (!confirm(t('approveConfirm', { name: request.song_name }))) {
      return
    }
    ...
    try {
      ...
      alert(t('approvedSuccess', { name: request.song_name }))
      fetchRequests()
    } catch (error: any) {
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-900">{t('songApprovalsTitle')}</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {requests.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-600">{t('noApprovalRequests')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4 text-sm text-gray-600">
                      {request.team_name && <div>{t('artistColon', { name: request.team_name })}</div>}
                      {request.key && <div>Key: {request.key}</div>}
                      {request.bpm && <div>BPM: {request.bpm}</div>}
                      {request.themes && request.themes.length > 0 && (
                        <div>{t('themeColon', { themes: request.themes.join(', ') })}</div>
                      )}
                    </div>
                    ...
                    <div className="text-sm text-gray-600">
                      ...
                      <p>{t('requestDateLabel', { date: new Date(request.created_at).toLocaleString('ko-KR') })}</p>
                    </div>
                    ...
                    {request.admin_notes && (
                      <div className="mt-2 p-3 bg-gray-50 rounded text-sm">
                        <strong>{t('adminNote')}</strong> {request.admin_notes}
                      </div>
                    )}
                  </div>
                    ...
                    {request.status === 'pending' && (
                      <>
                        <button
                          ...
                          title={!request.requester_id ? t('requesterDeletedCannotApprove') : ''}
                        >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-auto">
            ...
            <div className="p-6">
              ...
              {previewSong.lyrics && (
                <div className="mt-4">
                  <h3 className="font-bold mb-2">{t('lyricsLabel')}</h3>
