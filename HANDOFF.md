...
### 🟢 코드악보 + 연주 모드 통합 기능 — 6차 세션 대부분 완료 (브랜치 `feat/chord-chart`, **미배포**)
> 기능1(코드악보)·기능2(클릭)이 **하나로 통합**됨. 진입: 곡 → **악보 에디터(프레젠테이션 아이콘) → "코드악보" 버튼**.
> ⚠️ **`feat/chord-chart` 브랜치에만 있음(main 미병합·미배포).** 이어서: `git checkout feat/chord-chart`. 상세는 섹션 3.
> ⚠️ **테이블 SQL은 대표가 이미 적용함**(`20260722_song_chord_charts.sql`). 배포 시 재적용 확인.

**다음 할 일**
- [ ] **대표 최종 테스트** — 에디터 코드악보/편집/연주모드 전반, 저장 팀 공유, 세분/강세.
- [ ] **main 병합·배포** (원하면). 배포 전 `song_chord_charts` 테이블 프로덕션 적용 확인.
- [ ] **(요청) 원본 악보 뷰에도 클릭/하이라이트 적용 가능성 검토** — 아래 참고. 클릭(메트로놈)은 쉬움, 원본 위 하이라이트는 마디 좌표(bounding box) 추출 필요.
- [ ] (후속) 3박에 코드 있을 때 그 박 클릭 강세/전환음, WAV 내보내기, i18n(영문).

> **원본 악보 하이라이트 검토 메모**: 클릭 엔진(`useClickTrack`)은 화면과 독립이라 원본 뷰(`SheetMusicViewer`)에 재생 버튼만 붙이면 메트로놈은 바로 됨. 단 "현재 마디"를 **원본 이미지 위**에 강조하려면 마디별 픽셀 위치가 필요 → 비전 추출에 마디 bounding box 추가(작업↑·정확도 중간). 그래서 이번엔 **생성한 코드악보 위 하이라이트**로 구현함.
...
## 3. 최근 작업 (2026-07-22 6차 세션) — 코드악보 + 연주 모드 통합 ✅ (브랜치 `feat/chord-chart`, 미배포)

> 계획: `docs/내부/코드악보_자동변환_계획.md`, `연주모드_송폼클릭_악보싱크_계획.md`. 이어서: `git checkout feat/chord-chart`.

### 완료 (모두 로컬 dev 확인)
- [x] **코드악보 자동변환**: 악보(PDF/이미지) → 비전 LLM(`claude-sonnet-5`, **thinking 비활성** → 곡당 ~$0.016·~10초) → 마디별 코드/가사 JSON.
  - 라우트 `src/app/api/chord-chart/extract/route.ts`(songId→private버킷 다운로드/직접 이미지), 프롬프트/파서 `src/lib/chordChartExtract.ts`.
  - 섹션은 **인쇄된 라벨만** 잡음(추론 금지). 마디 내 여러 코드 + **박(beat) 위치**까지 추출.
- [x] **렌더** `ChordChartView`: 가로 선+barline, 코드는 **박 위치**에 표시, 가사 마디별 정렬, 인쇄 섹션 라벨.
- [x] **저장/공유**: `song_chord_charts` 테이블(곡당 1개, RLS=팀 격리) — **팀원이 같은 코드악보 공유·편집**. 마이그레이션 `20260722_song_chord_charts.sql`(대표 적용 완료).
- [x] **교정 편집** `ChordChartGridEditor`: 가로 그리드에서 **마디 탭 → 박자별 코드 슬롯(1·2·3·4)/가사/섹션** 수정 + 마디 추가/삭제 + 메타.
- [x] **연주 모드** `ChordChartPlayer`: **송폼대로 클릭 재생 + 현재 마디 하이라이트/자동스크롤**. 코드악보 없어도 클릭만 가능. 매핑 `src/lib/chordChartSections.ts`.
- [x] **메트로놈 옵션**: **세분(♩4분/♪8분)** + **박별 강세 선택**(`useClickTrack` 3단계 클릭음).
- [x] **에디터 통합**: 악보 에디터 헤더 "코드악보" 버튼 → 오버레이(코드악보/편집/연주). 탭 시 **집중 모드**(상단바 숨김). `ChordChartPanel`로 로직 묶음.
- [x] **일원화**: 곡목록의 별도 청록(코드악보)·🎵(클릭) 버튼 **제거** → 에디터 한 곳으로. `ChordChartModal`·`GuideTrackModal` 삭제.

### 핵심 파일
- `src/components/`: ChordChartView, ChordChartGridEditor, ChordChartPlayer, ChordChartPanel
- `src/hooks/useClickTrack.ts`, `src/lib/`: chordChartExtract, chordChartSections, songSection
- `src/app/api/chord-chart/extract/route.ts`, `supabase/migrations/20260722_song_chord_charts.sql`

### 남은 것
- 대표 최종 테스트 → main 병합·배포(테이블 프로덕션 적용 확인) → (요청)원본 악보 뷰 클릭/하이라이트 검토 → 후속(박 강세 클릭·WAV·i18n).

---

## 3-Y. 이전 작업 (2026-07-21 5차 세션) — 클릭 가이드 트랙 MVP ✅ (feat/guide-track, feat/chord-chart에 병합됨)

> **브랜치 `feat/guide-track` (미병합·미배포, origin에 푸시됨).** 이어서: `git checkout feat/guide-track`.
> 계획: `docs/내부/클릭_가이드트랙_계획.md`.

### 무엇
합주/리허설용 클릭(메트로놈) 가이드 트랙. 선택된 송폼을 가로 색상 타임라인으로 보여주고,
BPM·박자표에 맞춰 클릭 재생 + 섹션 전환 시 영어 음성 카운트인.

### 완료 (모두 로컬 dev 확인)
- [x] **Web Audio 클릭 엔진** `src/hooks/useClickTrack.ts` — look-ahead 스케줄러, 강/약박, 섹션별 마디, play(startIndex)/stop.
- [x] **모달 UI** `src/components/GuideTrackModal.tsx` — 가로 색상 타임라인(폭 ∝ 마디), 플레이헤드, BPM/박자표/음성큐, 섹션별 마디 수 편집.
- [x] **공용 모듈** `src/lib/songSection.ts` — 섹션 약어→계열/색상(hex+chip)/기본마디/영어명. **기능1 재활용 대상.**
- [x] **진입/게이팅** `main/components/SongListItem.tsx` — 🎵 버튼은 **송폼 선택 시에만 활성화**(아니면 회색+툴팁 "송폼을 선택하세요"). `form={songForms[song.id]}` 전달.
- [x] **음성 큐**: 영어 섹션명(I→Intro, V1→Verse 1, Pc→Pre-Chorus, C→Chorus…) + "one,two,three,four". 섹션명은 카운트 한 박 전에, TTS 지연 보정(`SPEECH_LEAD` 120ms 앞당김).
- [x] **타임라인 클릭 시크** — 파트 누르면 그 섹션부터 재생.
- [x] **시작 2마디 리드인** — 섹션명 → 카운트 → 첫 섹션 진입(시크에도 동일 적용).

### 남은 것 / 후속
- [ ] main 병합·배포 (원하면).
- [ ] i18n(영문) 추출 — 현재 한국어 인라인.
- [ ] 진입점 확대(콘티/미리보기) — 현재 메인 곡 리스트에만.
- [ ] (후속) WAV 내보내기 — 설계 문서 참조.
- [ ] TTS 타이밍 미세조정 여지(`SPEECH_LEAD`).

### 커밋 (feat/guide-track)
`aec7981`(MVP) → `ba7741c`(게이팅+타임라인) → `fc4fd1a`(영어큐) → `7efb542`(타이밍) → `4ba58b1`(시크+리드인)

---

## 3-Z. Storage 파일 접근 프록시 1~3단계 ✅ (main 배포 완료)
...
| 날짜 | 변경 |
|------|------|
| 2026-07-22 (6차) | **코드악보 + 연주 모드 통합** — 비전 추출(sonnet-5, thinking off) + `song_chord_charts` 저장(팀 공유) + 가로 그리드 교정편집(박자별 코드) + 송폼 클릭 재생/마디 하이라이트 + 메트로놈 세분·강세 + 악보 에디터 통합. 브랜치 `feat/chord-chart`(미배포). 테이블 `20260722_song_chord_charts.sql` |
| 2026-07-21 (5차) | **클릭 가이드 트랙 MVP** — Web Audio 메트로놈 + 송폼 가로 타임라인 + 영어 음성 카운트인 + 클릭 시크/2마디 리드인. 브랜치 `feat/guide-track`(미배포). 공용 `src/lib/songSection.ts` |
...
**현재 상태 (2026-07-22 6차 세션 종료 시점)**
- **냉동 해제.** 팀 단위 폐쇄 공유 모델. 요금제 전부 무료 전제.
- ✅ **Storage 프록시 1~3단계 (main 배포 완료)** — 파일 유출 구멍 닫힘.
- ✅ **코드악보 + 연주 모드 통합 기능 (브랜치 `feat/chord-chart`, 미배포)** — 비전 추출·저장(팀공유)·박자별 교정편집·송폼 클릭 재생·마디 하이라이트·메트로놈 세분/강세·악보 에디터 통합·진입 일원화. 상세 섹션 3.
- **⚠️ 이어서: `git checkout feat/chord-chart`** (코드악보/클릭 코드 전부 이 브랜치). `song_chord_charts` 테이블은 대표 적용 완료.
- **다음 착수**: 대표 최종 테스트 → main 병합·배포(테이블 프로덕션 적용 확인) → (요청)원본 악보 뷰 클릭/하이라이트 검토 → 후속(박 강세 클릭·WAV·i18n). + JWT-in-URL 강화(선택).
