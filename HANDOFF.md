...
**마지막 업데이트**: 2026년 7월 15일 (3차 세션)

---

## ⏭️ 다음 세션 즉시 할 일 (3차 세션 종료 시점)

**세션 재시작 사유: Supabase MCP를 붙이는 중.** 대표가 아래를 실행한 뒤 세션을 다시 시작함:
```
claude mcp add --transport http supabase https://mcp.supabase.com/mcp   # 그다음 /mcp 로 인증
```
→ MCP가 붙으면 **캘린더 기능 1단계(DB 스키마)부터** 착수. Claude가 마이그레이션을 직접 적용·검증 가능.
- 계획서: `docs/내부/팀_일정_캘린더_계획.md` (확정본, 8번 섹션의 6단계)
- 1단계 = `team_events` + `team_event_attendance` 테이블 + RLS 마이그레이션 작성/적용.
- MCP가 안 붙었으면: Claude가 SQL 파일만 쓰고 대표가 SQL Editor에서 직접 실행(RLS 때와 동일).
...
> **2026-07-15 (3차)**: 메인 레이아웃 팀 단위로 전환 + 캘린더 기능 계획 확정. (아래 섹션 3)
...
## 3. 최근 작업 (2026-07-15 3차 세션) — 메인 레이아웃 팀 전환 + 캘린더 계획 ✅
...
## 3-B. 이전 작업 (2026-07-14 2차 세션) — 1단계 anon 차단 적용 완료 ✅
...
### 🗓️ 캘린더 기능 — 1단계부터 (**다음 세션 최우선**)

계획서 8번 6단계. MCP 붙었으면 Claude가 DB 직접 적용.
1. [ ] **1단계 DB**: `team_events` + `team_event_attendance` 마이그레이션 + RLS 작성/적용
2. [ ] 2단계: 타입(`types/teamEvent.ts`) + 훅(`useTeamEvents`, `useEventAttendance`)
3. [ ] 3단계: `my-team/[id]`에 `일정` 탭 + 월/아젠다 뷰
4. [ ] 4단계: 생성/수정 모달 + 반복(인스턴스 12주치, 이 회차/이후/전체 수정)
5. [ ] 5단계: 참석 토글 + 요약 집계 + 사유
6. [ ] 6단계: 예배 일정 ↔ team_setlists 콘티 연결
...
**현재 상태 (2026-07-15 3차 세션 종료 시점)**
- **냉동 해제.** 팀 단위 폐쇄 공유 모델로 전환 작업 중.
- **모델**: 권리사 협상을 피하기 위해, 팀이 자기 악보를 올려 그 팀만 보는 구조. 운영자는 호스팅만.
- **요금제**: 전부 무료 전제.
- **1단계(anon 차단 RLS) 적용 완료** + **메인 레이아웃 팀 3분기 전환 완료** (푸시됨).
- **다음 착수: 🗓️ 캘린더 기능 1단계(DB 스키마)** — 섹션 최상단 "다음 세션 즉시 할 일" 참조.
- ⚠️ **Storage 버킷이 아직 public** — 파일은 여전히 URL로 새고 있음(섹션 5 B단계). 캘린더와 별개로 남아있음.

### 다음 세션 착수 팁
- **Supabase MCP를 붙이는 중.** `claude mcp add --transport http supabase https://mcp.supabase.com/mcp` → `/mcp` 인증 → 세션 재시작.
  붙으면 라이브 DB 직접 쿼리/수정 가능(캘린더 마이그레이션 직접 적용·검증).
- MCP 없이도 **anon 키(`.env.local`)로 REST 직접 호출해 검증 가능** (2·3차 세션에서 이 방법 사용).
- 로컬 dev 서버는 `/dev`로 재실행. 3차 세션 중 `npm run build` 때문에 종료된 상태.
