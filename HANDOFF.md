...
### 🟡 새 기능
- [x] **기능2: 클릭(가이드) 트랙 MVP 완료** — 브랜치 `feat/guide-track`(**미병합·미배포**). 상세는 섹션 3.
  이어서: `git checkout feat/guide-track`. 배포하려면 main 병합.
- [ ] **기능1: 악보→코드악보 자동변환** — `docs/내부/코드악보_자동변환_계획.md`. 비전 LLM 추출+교정 UI. 미결 5개(입력소스/모델비용/저장구조/PDF 포함여부).
  ★ **연동 지점**: 기능2가 쓰는 섹션별 마디 수를 기능1(코드악보)이 자동으로 채우도록 설계됨. 색상/기본마디/영어명 로직은 `src/lib/songSection.ts` 공용 모듈에 있음(기능1 재활용).
...
## 3. 최근 작업 (2026-07-21 5차 세션) — 클릭 가이드 트랙 MVP ✅ (브랜치)
...
- **기능2(클릭 가이드 트랙) MVP 완료** — 브랜치 `feat/guide-track`(미병합·미배포). `git checkout`으로 이어서.
- **다음 착수**: 기능1(악보→코드악보 자동변환) — 설계 문서 작성됨, 미결 항목 답 대기. 기능2와 마디 수로 연동. + JWT-in-URL 강화 여부 결정(선택).
