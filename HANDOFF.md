...
### 🔴 최우선 — Storage 파일 프록시 **3단계(버킷 private 전환)** ← 지금 여기
> 5차 세션에서 **1~2단계 완료**(프록시 라우트+배선), 브라우저 검증 일부 통과. 아래 섹션 3 참조.
> ⚠️ **작업은 `feat/file-proxy-signed-url` 브랜치에 있음(커밋 `55d36c6`). 아직 main 미병합.**
> 이어서 하려면 **`git checkout feat/file-proxy-signed-url`** 먼저.

A. [ ] **남은 브라우저 검증** — PDF 타입 곡(pdfjs 경로) + 다운로드(fetch 경로) 2개만 302 확인하면 2단계 완전 검증. (이미지 렌더·뷰어는 302→200 확인됨)
B. [ ] **(선택) 브랜치 푸시/배포** — 버킷 public이라 안전. 프로덕션에서도 302 확인.
C. [ ] **3단계 적용** (검증 후에만, 되돌리기 어려움):
   1. `storage.buckets` 둘 다 `public=false`
   2. `sheetmusic_select_public` 정책 삭제
   3. `src/lib/fileUrl.ts`의 public fallback 제거 + 토큰 항상 존재 보장
   → 이 순간 파일 유출 구멍이 닫힘("팀끼리만 본다" 모델 성립).
D. [ ] **(결정 필요) JWT-in-URL 강화 여부** — 현재 `?token=`에 실제 access token 노출. 3단계 후 실노출 시작. 그대로 둘지 / HMAC 서명 grant 토큰으로 바꿀지 결정.
...
> **브랜치**: `feat/file-proxy-signed-url` (커밋 `55d36c6`) — **미푸시·미병합**. 이어서: `git checkout feat/file-proxy-signed-url`.
...
### 남은 것 = 3단계 (섹션 최상단 "다음 세션 할 일" A~D 참조)
- 남은 검증(PDF·다운로드) → 3단계 SQL(버킷 private + 정책 삭제 + fallback 제거) → JWT-in-URL 강화 결정.
...
| 대상 | 문제 | 단계 |
|---|---|---|
| **Storage 버킷 2개** | `public: true` → file_url만 알면 **로그인 없이 악보 파일 다운로드** (HTTP 200 실측). ⚠️ **프록시 배관 완료(5차), private 전환만 남음** | B |
...
- **요금제**: 전부 무료 전제.
- **완료**: 팀전환 1단계(anon RLS) + 메인 레이아웃 팀 3분기 + 팀 일정 캘린더 전체 + **Storage 파일 프록시 1~2단계**.
- **진행 중: Storage 파일 프록시** — 1~2단계(배관+배선) 완료·검증 일부. **브랜치 `feat/file-proxy-signed-url`(미푸시). 이어서: `git checkout` 후 3단계.**
- **다음 착수**: 섹션 최상단 "다음 세션 할 일" A~D (남은 검증 → 버킷 private 전환 → JWT-in-URL 강화 결정).
- ⚠️ **Storage 버킷은 여전히 public** — 프록시 배관은 깔렸지만 **private 스위치(3단계)를 아직 안 내려서 파일은 계속 URL로 샘.** 3단계가 진짜 마무리.
